...
p <- p + geom_line(data = d[d$item == "Statutory Debt Limit", ],
                   ...
                   size = 2, colour = "black")
p + ylim(c(0, 1.75e+07)) + ggtitle("Total Public Debt Subject to Limit") + ylab("Opening Balance of Debt")
...
svg("debt_ceiling_inflation_adjusted.svg", height = 9, width = 12)
...
svg("debt_ceiling_inflatin_adjusted_diff.svg", height = 9, width = 12)

qplot(data = d[d$item %in% c("Statutory Debt Limit", "Total Public Debt Subject to Limit"),],
      x = date, y = open_mo, geom = "line", colour = desc(item), size = I(3),
      ylab = "Total Public Debt Subject to Limit: Inflation Adjusted") +
  theme(axis.title=element_text(face="bold.italic", size="12", color="brown"),
        legend.position="top")
...
p <- p + geom_area(fill = I('green'), alpha = I(.5), size = 2) + geom_line(colour = I("white"))
p <- p + geom_segment(aes(x = as.Date(round_any(as.POSIXct(date), 60*60*24*7*3, floor)),
                          xend = as.Date(round_any(as.POSIXct(date), 60*60*24*7*3, floor)),
                          y = 0, yend = open_mo / cpi), size = .2, colour = "white")
p <- p + geom_line(data = d[d$item == "Statutory Debt Limit", ],
                   aes(x = date, y = open_mo / cpi, colour = item),
                   size = 2, colour = "black")
p + ylim(0, 75000) + ggtitle("Total Public Debt Subject to Limit") + ylab("Opening Balance of Debt")
...
sql <- 'select * from "t3a" where date > "2000-10-01"'
d <- sqldf(sql)
d$date <- as.Date(d$date)
d$month <- substring(d$date, 1, 7)

ind <- (d$item == "Total Redemptions") | (d$item == "Total Issues")
qplot(data = d[ind, ], x = month, y = mtd, fill = item, geom = "bar", stat = "identity")

# try a stream graph
ind <- (d$item == "Total Redemptions") | (d$item == "Total Issues")

## -- match the dates of redemptions (a bit less) and issues

#d$today[d$item == "Net Change in Public Debt Outstanding"]
...
# stack
df.stack <- df[,-1]
df.stack <- df.stack[,rev(sort(colnames(df.stack)))]
df.stack <- apply(df.stack, 1, cumsum)
df.stack <- apply(df.stack, 1, function(x) sapply(x, cumsum))
df.stack <- t(apply(df.stack, 1, function(x) x - mean(x)))

# smooth the curves
df.density <- apply(df.stack, 2, function(x) spline(x = df[,1], y = x)) # smooth each item
df.id <- sort(rep(colnames(df.stack), each = length(df.density[[1]][[1]])))
df.density <- do.call("rbind", lapply(df.density, as.data.frame))
df.density <- data.frame(df.density, id = df.id)

df.smooth <- data.frame(x = unlist(tapply(df.density$x, df.density$id, function(x) c(x, rev(x)))),
                        y = c(apply(unstack(df.density[,2:3]), 1, min), unlist(tapply(df.density$y, df.density$id, function(x) c(rev(x), x)))[1:(table(df.density$id)[1]+2*max(cumsum(table(df.density$id))[-dim(df.stack)[2]]))]), id = rep(names(table(df.density$id)), each = unique(2*table(df.density$id))))

qplot(x = x, y = y, data = df.smooth, geom = "polygon", fill = id)

## -- see geom_polygon docs
...
# before I smooth, let's see if the dual polygons are working
positions <- data.frame(date = rep(c(df$date, rev(df$date)), times = 2),
                        values = c(df$redemption, rev(df$net), df$net, rev(df$issues)),
                        ids = factor(c(rep("redemption", length = 2*dim(df)[1]), rep("issues", length = 2*dim(df)[1]))))
#levels(positions$ids) <- c("redemption", "issues")

# plot!
svg("ledger_unsmoothed.svg", height = 9, width = 12)

ggplot(data = positions, aes(x = date, y = values)) +
  geom_polygon(aes(fill = ids, order = desc(ids))) +
  scale_fill_brewer(palette = "Set1") +
  geom_line(data = df, aes(x = date, y = net), colour = I("white"), inherit.aes = FALSE)

dev.off()

## now let's try with smooth
# lowess vs spline
# http://stats.stackexchange.com/questions/20032/comparing-smoothing-splines-vs-loess-for-smoothing
